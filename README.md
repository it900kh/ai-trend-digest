...
AI関連のニュースをRSSから自動収集し、Gemini API を使って**非エンジニア向けに2〜3行**へ要約します。結果は SQLite に保存し、サイトが読めるJSONへ書き出します。

## 仕組み

```
RSS収集 → SQLite保存 → Geminiで要約 → digest.json 書き出し → サイトが表示
（重複はURLで自動スキップ）
```
...
```bash
npm install
export GEMINI_API_KEY="..."   # APIキーを設定
npm start                     # 収集→要約→書き出しを一括実行
```
...
JSONだけ書き出し直したい場合は `npm run export` を実行します。
...
| `src/summarize.js` | Gemini APIでやさしい要約を生成 |
...
Settings → Secrets に `GEMINI_API_KEY` を登録すれば、
